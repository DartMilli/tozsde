...
def temp_dirs():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        yield {"log_dir": log_dir, "backup_dir": backup_dir, "data_dir": data_dir}
...
def mock_config(temp_dirs):
    ...
    config.DATA_DIR = temp_dirs["data_dir"]
...
def scheduler(mock_config):
    ...
    return CronTaskScheduler(settings=mock_config)
...
def test_daily_pipeline_success(scheduler, mock_config):
    ...
    scheduler.backup_manager.backup_database = Mock(
        return_value={"success": True, "size_mb": 10.5}
    )
    scheduler.backup_manager.cleanup_old_backups = Mock(
        return_value={"deleted_count": 2}
    )
    scheduler.log_manager.rotate_logs = Mock(return_value={"archived_count": 3})

    with patch(
        "app.infrastructure.cron_tasks.DataManagerRepository",
        return_value=mock_data_manager,
    ), patch(
        "app.decision.decision_engine.DecisionEngine", return_value=mock_decision_engine
    ):
...
def test_daily_pipeline_partial_failure(scheduler):
    ...
    scheduler.log_manager.rotate_logs = Mock(return_value={"archived_count": 0})
...
def test_weekly_audit_success(scheduler):
    ...
    class MockBacktestAuditor:
        ...
        def run_weekly_audit(self):
            return {
                "success": True,
                "metrics": {"total_trades": 50, "win_rate": 0.65, "sharpe_ratio": 1.8},
            }
...
def test_monthly_optimization_success(scheduler):
    ...
    class MockGeneticOptimizer:
        ...
        def run_monthly_optimization(self):
            return {"success": True, "best_fitness": 0.85, "generations": 100}
...
def test_monthly_optimization_failure(scheduler):
    ...
    class MockGeneticOptimizer:
        ...
        def run_monthly_optimization(self):
            return {"success": False, "best_fitness": 0.0, "generations": 0}
...
def test_log_cron_execution(scheduler, mock_config):
    ...
    scheduler._log_cron_execution(
        task_name="test_task", status="success", duration=10.5, details={"test": "data"}
    )
...
def test_log_cron_execution_handles_write_error(scheduler, mock_config):
    ...
    scheduler._log_cron_execution(task_name="test_task", status="success", duration=1.0)
