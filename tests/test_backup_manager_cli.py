...
def _create_test_db(path: Path, size_kb=100):
    ...
    for i in range(rows_needed):
        cursor.execute(
            ...
            (f"TEST_{i}", f"2023-01-{(i%28)+1:02d}", 100.0+i, 101.0+i, 99.0+i, 100.5+i, 1000000+i)
        )
...
class TestMainCLIIntegration:
    ...
    def test_main_backup_coverage(self, tmp_path):
        ...
        with mock.patch('sys.argv', test_argv):
            # Mock Config.DB_PATH to use test database
            with mock.patch('app.infrastructure.backup_manager.Config.DB_PATH', db_path):
                # Mock the backup directory initialization
                with mock.patch('app.infrastructure.backup_manager.BackupManager.__init__', 
                               return_value=None):
                    manager = BackupManager(db_path=db_path, backup_dir=backup_dir)
                    
                    # Mock backup_database to return success
                    with mock.patch.object(manager, 'backup_database', 
                                          return_value={'success': True, 'backup_path': str(backup_dir / 'test.db'), 'size_mb': 1.5}):
                        
                        # Capture stdout
                        captured_output = StringIO()
                        with mock.patch('sys.stdout', captured_output):
                            with mock.patch('app.infrastructure.backup_manager.BackupManager', return_value=manager):
                                try:
                                    main()
                                except SystemExit:
                                    pass
    ...
    def test_main_cleanup_coverage(self, tmp_path):
        ...
        with mock.patch('sys.argv', test_argv):
            with mock.patch('app.infrastructure.backup_manager.BackupManager.__init__', 
                ...
                with mock.patch.object(manager, 'cleanup_old_backups', 
                                      return_value={'deleted_count': 2, 'bytes_freed': 5242880, 'errors': []}):
    ...
    def test_main_cleanup_with_errors_coverage(self, tmp_path):
        ...
        with mock.patch('sys.argv', test_argv):
            with mock.patch('app.infrastructure.backup_manager.BackupManager.__init__', 
                ...
                with mock.patch.object(manager, 'cleanup_old_backups', 
                                      return_value={'deleted_count': 1, 'bytes_freed': 1048576, 'errors': ['Permission denied on file.db']}):
    ...
    def test_main_stats_coverage(self, tmp_path):
        ...
        with mock.patch('sys.argv', test_argv):
            with mock.patch('app.infrastructure.backup_manager.BackupManager.__init__', 
                ...
                with mock.patch.object(manager, 'get_backup_stats', return_value=stats_data):
    ...
    def test_main_list_coverage(self, tmp_path):
        ...
        with mock.patch('sys.argv', test_argv):
            with mock.patch('app.infrastructure.backup_manager.BackupManager.__init__', 
                ...
                # Mock list_backups
                backups_data = [
                    {'filename': 'backup_001.db', 'size_mb': 50.0, 'created': '2023-01-01', 'age_days': 10},
                    {'filename': 'backup_002.db', 'size_mb': 55.0, 'created': '2023-01-05', 'age_days': 5},
                ]
                with mock.patch.object(manager, 'list_backups', return_value=backups_data):
    ...
    def test_main_verify_success_coverage(self, tmp_path):
        ...
        with mock.patch('sys.argv', test_argv):
            with mock.patch('app.infrastructure.backup_manager.BackupManager.__init__', 
                ...
                with mock.patch.object(manager, 'verify_backup', 
                                      return_value={'valid': True, 'tables': 5}):
                    ...
                    output = captured_output.getvalue()
                    assert ('Backup valid' in output or 'valid' in output.lower()) or output == ''
    ...
    def test_main_restore_success_coverage(self, tmp_path):
        ...
        with mock.patch('sys.argv', test_argv):
            with mock.patch('app.infrastructure.backup_manager.BackupManager.__init__', 
                ...
                with mock.patch.object(manager, 'restore_backup', 
