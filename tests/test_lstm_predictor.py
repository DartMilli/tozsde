...
class TestLSTMMarketPredictor:
    ...
    def test_model_training(self, predictor, sample_data):
        ...
        history = predictor.train(X, y, epochs=5, batch_size=16)
    ...
    def test_training_loss_decrease(self, predictor, sample_data):
        ...
        history = predictor.train(X, y, epochs=20, batch_size=16)
        
        # Loss should generally decrease (allow some noise)
        initial_loss = np.mean(history['loss'][:3])
        final_loss = np.mean(history['loss'][-3:])
        
        # Allow 20% increase due to randomness, but generally should decrease
