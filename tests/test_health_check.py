...
def test_check_all_healthy(mock_disk, mock_memory, mock_cpu, health_checker, temp_dirs):
    ...
    mock_memory.return_value = Mock(percent=50.0, available=4*1024**3, total=8*1024**3)
    ...
    # Mock API check
    with patch.object(health_checker, 'check_api_health', return_value={
        'healthy': True, 'status_code': 200, 'message': 'OK'
    }):
...
def test_check_all_with_issues(health_checker):
    ...
    # Mock failing API
    with patch.object(health_checker, 'check_api_health', return_value={
        'healthy': False, 'message': 'API down'
    }):
        with patch.object(health_checker, 'check_database_health', return_value={
            'healthy': False, 'message': 'DB locked'
        }):
            with patch.object(health_checker, 'check_disk_space', return_value={
                'healthy': True, 'message': 'OK'
            }):
                with patch.object(health_checker, 'check_memory_usage', return_value={
                    'healthy': True, 'message': 'OK'
                }):
                    with patch.object(health_checker, 'check_cpu_usage', return_value={
                        'healthy': True, 'message': 'OK'
                    }):
                        with patch.object(health_checker, 'check_cron_execution', return_value={
                            'healthy': True, 'message': 'OK'
                        }):
                            result = health_checker.check_all()
...
def test_get_recent_health_logs(health_checker, temp_dirs):
    ...
    # Write 3 log entries
    for i in range(3):
        time_offset = datetime.utcnow() - timedelta(hours=i)
        log_entry = {
