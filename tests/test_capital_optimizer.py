...
def optimizer():
    ...
    return CapitalUtilizationOptimizer(
        total_capital=100000.0, max_position_pct=0.05, min_position_size=100.0
    )
...
class TestKellyCriterion:
    ...
    def test_kelly_with_winning_odds(self, optimizer):
        """Test Kelly fraction with winning trading odds (half-Kelly dampening applied)."""
        # 60% win rate, avg win $100, avg loss $100
        kelly = optimizer.calculate_kelly_fraction(
            win_rate=0.60, avg_win=100.0, avg_loss=100.0
        )

        # Full Kelly = (0.6 * 100 - 0.4 * 100) / 100 = 0.20
        # After half-Kelly multiplier (0.5): 0.20 * 0.5 = 0.10
    ...
    def test_kelly_with_losing_odds(self, optimizer):
        ...
        kelly = optimizer.calculate_kelly_fraction(
            win_rate=0.40, avg_win=100.0, avg_loss=100.0
        )
    ...
    def test_kelly_with_different_payoffs(self, optimizer):
        ...
        kelly = optimizer.calculate_kelly_fraction(
            win_rate=0.50, avg_win=150.0, avg_loss=100.0
        )

        # Kelly % = (0.5 * 150 - 0.5 * 100) / 150 = 0.167, after half-Kelly: 0.0835
        assert kelly == pytest.approx(0.0835, rel=0.02)

    def test_kelly_clamped_to_max(self, optimizer):
        """Test Kelly is clamped to maximum of 0.50."""
        # Very high win rate
        kelly = optimizer.calculate_kelly_fraction(
            win_rate=0.99, avg_win=1000.0, avg_loss=1.0
        )

        # Even with very high odds, after half-Kelly and cap: max 0.50
        assert kelly <= 0.50

    def test_kelly_with_zero_avg_loss(self, optimizer):
        """Test Kelly with zero average loss returns 0."""
        kelly = optimizer.calculate_kelly_fraction(
            win_rate=0.50, avg_win=100.0, avg_loss=0.0
        )
...
class TestPositionSizing:
    ...
    def test_optimal_position_basic(self, optimizer):
        ...
        pos = optimizer.calculate_optimal_position_size(
            ticker="AAPL", kelly_fraction=0.25, volatility=0.02
        )
    ...
    def test_position_respects_max_limit(self, optimizer):
        ...
        pos = optimizer.calculate_optimal_position_size(
            ticker="TEST", kelly_fraction=0.50, volatility=0.01
        )
    ...
    def test_position_with_high_volatility(self, optimizer):
        ...
        pos_low_vol = optimizer.calculate_optimal_position_size(
            ticker="TEST", kelly_fraction=0.05, volatility=0.01
        )

        pos_high_vol = optimizer.calculate_optimal_position_size(
            ticker="TEST", kelly_fraction=0.05, volatility=0.20
        )
    ...
    def test_position_respects_min_size(self, optimizer):
        ...
        pos = optimizer.calculate_optimal_position_size(
            ticker="TEST", kelly_fraction=0.01, volatility=0.20
        )
    ...
    def test_portfolio_weight_calculation(self, optimizer):
        ...
        pos = optimizer.calculate_optimal_position_size(
            ticker="TEST", kelly_fraction=0.25, volatility=0.02
        )
...
class TestCapitalAllocationOptimization:
    ...
    def test_utilization_rate(self, optimizer):
        ...
        positions = {"AAPL": {"kelly": 0.25, "volatility": 0.02}}
...
class TestEdgeCases:
    ...
    def test_zero_kelly(self, optimizer):
        ...
        pos = optimizer.calculate_optimal_position_size(
            ticker="TEST", kelly_fraction=0.0, volatility=0.02
        )
    ...
    def test_very_low_volatility(self, optimizer):
        ...
        pos = optimizer.calculate_optimal_position_size(
            ticker="TEST", kelly_fraction=0.25, volatility=0.001
        )
