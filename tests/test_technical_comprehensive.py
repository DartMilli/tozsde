...
class TestRSI:
    ...
    def test_rsi_basic(self):
        ...
        assert np.all(
            (result[~np.isnan(result)] >= 0) & (result[~np.isnan(result)] <= 100)
        )
...
class TestMACD:
    ...
    def test_macd_custom_params(self):
        ...
        macd_line, signal_line, histogram = macd(data, fast=8, slow=21, signal=5)
...
class TestBollingerBands:
    ...
    def test_bbands_custom_std(self):
        ...
        if np.any(valid_idx):
            assert np.all(
                (upper_3[valid_idx] - middle_3[valid_idx])
