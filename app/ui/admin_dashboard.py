...
def get_drawdown_analysis():
    ...
    try:
        ...
        response = {
            ...
            'drawdown_start': dd_analysis.drawdown_start.isoformat() if dd_analysis.drawdown_start else None,
            'drawdown_end': dd_analysis.drawdown_end.isoformat() if dd_analysis.drawdown_end else None,
            'recovery_date': dd_analysis.recovery_date.isoformat() if dd_analysis.recovery_date else None,
...
def get_rolling_performance():
    ...
    try:
        ...
        rolling = analytics.calculate_rolling_metrics(returns, dates, window_days=window)
...
def get_capital_utilization():
    ...
    try:
        ...
        if not history:
            return jsonify({
                'message': 'No capital utilization data available'
            }), 200
        
        # Calculate summary metrics
        recent_positions = history[:10] if len(history) >= 10 else history
        
        avg_utilization = sum(p['portfolio_weight'] for p in recent_positions) / len(recent_positions)
        avg_kelly = sum(p['kelly_fraction'] for p in recent_positions) / len(recent_positions)
...
def get_strategy_performance():
    ...
    try:
        ...
        for strategy in strategies:
            stats = analyzer.analyze_strategy_performance(strategy, days_back=days)
            if stats:
