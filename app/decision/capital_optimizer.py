"""
Capital Utilization Optimizer - Optimizes capital usage across trades.

This module implements capital optimization strategies including position sizing,
diversification limits, and margin utilization tracking to ensure optimal
allocation of available capital while managing risk.

Classes:
    PositionSize: Dataclass for position sizing result
    CapitalUtilizationOptimizer: Main optimizer class
"""
...
class CapitalAllocation:
    ...
    positions: Dict[str, float]  # {ticker: size}
...
class CapitalUtilizationOptimizer:
    """
    Optimizes capital allocation across trades using Kelly criterion
    and risk-weighted position sizing.
    
    Key features:
    - Kelly criterion for optimal position sizing
    - Max position limits (e.g., 5% per trade)
    ...
    def __init__(
        ...
        """
        Initialize the capital optimizer.
    ...
    def _init_db(self) -> None:
        """Initialize SQLite database for capital optimization tracking."""
        if not self.db_path:
            return
    ...
    def calculate_kelly_fraction(
        ...
        where W = win rate, L = loss rate, Avg_W/L = average win/loss
        ...
        Returns:
            float: Kelly fraction as decimal (e.g., 0.25 = 25%)
        """
        ...
        # Clamp to sensible range [0.01, 0.50]
    ...
    def calculate_optimal_position_size(
        ...
        Args:
            ...
            max_position_override: Override default max position limit
        
        Returns:
            PositionSize: Position sizing result
        """
        # Risk-adjusted Kelly (reduces kelly by volatility to account for market conditions)
        ...
        # Hard max position limit
        max_limit = max_position_override or (self.total_capital * self.max_position_pct)
    ...
    def optimize_capital_allocation(
        self,
        positions: Dict[str, Dict],  # {ticker: {kelly, volatility, expected_return}}
        rebalance: bool = False
    ) -> CapitalAllocation:
        """
        Optimize capital allocation across multiple positions.
        
        Args:
            positions: Dict of {ticker: {kelly, volatility, expected_return}}
            rebalance: Whether to rebalance to target allocations
        
        Returns:
            CapitalAllocation: Optimization result
        """
        allocated_positions = {}
        total_allocated = 0.0
        ...
        # Calculate diversification score (lower = more concentrated)
        diversification_score = self._calculate_diversification_score(allocated_positions)
        
        unused_capital = max(0.0, self.total_capital - total_allocated)
        utilization_rate = total_allocated / self.total_capital if self.total_capital > 0 else 0.0
    ...
    def _calculate_diversification_score(self, positions: Dict[str, float]) -> float:
        ...
        Score of 0.0 = perfectly diversified, 1.0 = single position
        
        Args:
            positions: Dict of {ticker: size}
        
        Returns:
            float: Diversification score (0.0-1.0)
        """
        if not positions:
            return 0.0
    ...
    def _persist_position_size(self, position: PositionSize) -> None:
        """Persist position size to database."""
        if not self.db_path:
            return
    ...
    def _persist_capital_allocation(self, allocation: CapitalAllocation) -> None:
        """Persist capital allocation to database."""
        if not self.db_path:
            return
    ...
    def get_position_history(self, ticker: str = None) -> List[Dict]:
        """
        Get position sizing history from database.
    ...
    def estimate_max_drawdown(self, positions: Dict[str, float], volatility_avg: float) -> float:
        """
        Estimate potential maximum drawdown.
        
        Args:
            positions: Dict of {ticker: size}
            volatility_avg: Average volatility across positions
        
        Returns:
            float: Estimated max drawdown percentage
        """
        # Rough estimate: max drawdown ≈ 3 * volatility (99.7% confidence)
        # multiplied by utilization rate
        
        utilization = sum(positions.values()) / self.total_capital if self.total_capital > 0 else 0
        
        estimated_drawdown = 3.0 * volatility_avg * utilization
        
        return min(1.0, estimated_drawdown)  # Cap at 100%
