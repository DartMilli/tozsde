...
class LogManager:
    ...
    def rotate_logs(self) -> Dict:
        ...
        """
        logger.info(f"Starting log rotation (age threshold: {self.ROTATION_AGE_DAYS} days)...")
        
        cutoff_date = datetime.utcnow() - timedelta(days=self.ROTATION_AGE_DAYS)
        rotated_files = []
        errors = []
        bytes_saved = 0
        ...
        try:
                ...
                try:
                    ...
                    if mtime < cutoff_date:
                        ...
                        if archive_path:
                            ...
                            logger.info(f"Rotated: {log_file.name} -> {archive_path.name} "
            ...
            logger.info(f"Log rotation complete: {len(rotated_files)} files rotated, "
    ...
    def cleanup_old_archives(self) -> Dict:
        ...
        """
        logger.info(f"Starting archive cleanup (retention: {self.ARCHIVE_RETENTION_DAYS} days)...")
        ...
        try:
            ...
            logger.info(f"Archive cleanup complete: {len(deleted_files)} files deleted, "
    ...
    def get_log_stats(self) -> Dict:
        ...
        try:
            ...
            if oldest_file:
                oldest_datetime = datetime.fromtimestamp(oldest_mtime)
                oldest_age_days = (datetime.utcnow() - oldest_datetime).total_seconds() / 86400
            ...
            return {
                ...
                'oldest_age_days': round(oldest_age_days, 1) if oldest_age_days else None,
    ...
    def _compress_log(self, log_file: Path) -> Optional[Path]:
        ...
        """
        try:
            archive_path = log_file.with_suffix(log_file.suffix + self.ARCHIVE_EXTENSION)
...
def main():
    ...
    parser.add_argument('--force-rotate', action='store_true', help='Force rotate all logs')
    parser.add_argument('--force-cleanup', action='store_true', help='Force cleanup all archives')
    ...
    if args.rotate:
        ...
        print(f"Rotated {result['rotated_count']} files, saved {result['bytes_saved']} bytes")
        if result['errors']:
            print(f"Errors: {result['errors']}")
    
    elif args.cleanup:
        result = manager.cleanup_old_archives()
        print(f"Deleted {result['deleted_count']} files, freed {result['bytes_freed']} bytes")
        if result['errors']:
            print(f"Errors: {result['errors']}")
        ...
        print(f"  Oldest file: {stats['oldest_file']} ({stats['oldest_age_days']} days old)")
