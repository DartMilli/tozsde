...
class CronTaskScheduler:
    ...
    def run_daily_pipeline(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"Daily pipeline completed in {duration:.2f}s - "
            ...
            self._log_cron_execution(
                task_name=task_name, status="error", duration=duration, error=str(e)
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Cron task scheduler for trading system"
    )
    ...
    parser.add_argument(
        "--monthly", action="store_true", help="Run monthly optimization"
    )
