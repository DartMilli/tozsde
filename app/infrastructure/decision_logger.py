...
class NoTradeReason(Enum):
    ...
    HIGH_CORRELATION = "HIGH_CORRELATION"  # correlation with existing positions too high
    ...
    DIVERSIFICATION_CONSTRAINT = "DIVERSIFICATION_CONSTRAINT"  # would violate diversification limits
    RISK_THRESHOLD_EXCEEDED = "RISK_THRESHOLD_EXCEEDED"  # portfolio risk already too high
...
class NoTradeDecisionLogger:
    ...
    def log_no_trade_decision(self, decision: NoTradeDecision) -> bool:
        ...
        try:
            ...
            logger.info(f"Logged no-trade decision for {decision.ticker}: {decision.reason.value}")
            return True
        except sqlite3.Error as e:
    ...
    def export_no_trade_decisions(
        self,
        output_file: str,
        days_back: int = 30
    ) -> bool:
        """
        Export no-trade decisions to JSON file.
        ...
        """
        decisions = self.get_no_trade_decisions(days_back=days_back)
        
        try:
            with open(output_file, 'w') as f:
                json.dump(decisions, f, indent=2, default=str)
            
            logger.info(f"Exported {len(decisions)} no-trade decisions to {output_file}")
            return True
    ...
    def clear_old_decisions(self, days_to_keep: int = 90) -> int:
        ...
        try:
            ...
            logger.info(f"Deleted {deleted_count} no-trade decisions older than {days_to_keep} days")
