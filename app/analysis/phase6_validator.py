...
class Phase6Validator:
    ...
    def _check_effectiveness(self, ticker: str) -> Dict:
        DecisionEffectivenessAnalyzer().compute_for_range(ticker=ticker)
        query = """
    ...
    def _check_position_sizing(self) -> Dict:
        sizer = PositionSizer(max_position_pct=Config.P6_POSITION_MAX_PCT)
        base = 1000.0
        equity = 10000.0
        low = sizer.compute(base, confidence=0.2, wf_score=0.5, safety_discount=0.0, equity=equity)
        high = sizer.compute(base, confidence=0.8, wf_score=0.9, safety_discount=0.0, equity=equity)
        capped = sizer.compute(base * 100, confidence=1.0, wf_score=1.0, safety_discount=0.0, equity=equity)
        ...
        return {
            ...
            "cap_enforced": capped.final_size <= equity * Config.P6_POSITION_MAX_PCT,
        }
    ...
    def _check_promotion_gate(self) -> Dict:
        ...
        candidate_metrics = {
            "wf_stability": Config.P6_WF_STABILITY_BASELINE,
            "max_drawdown": Config.P6_MAX_DRAWDOWN_BASELINE,
            "effectiveness": 0.0,
